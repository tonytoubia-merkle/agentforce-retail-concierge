...
function buildWelcomeMessage(ctx: CustomerSessionContext): string {
  ...
  if (isAppended) {
    ...
    if (ctx.email) lines.push(`[INSTRUCTION] The customer has been identified. When calling any action that requires a contactId (Create_Meaningful_Event, Update_Contact_Profile, etc.), pass "${ctx.contactId || ctx.email}" as the contactId input. If the contactId starts with "003", pass it directly. Otherwise pass the email address — the flow will resolve it automatically.`);
